...
const LikeDislike = (props) => {
    ...
    useEffect(()=>{
        ...
        axios.post(`${domain}/likeDislike/getDisLike`, variables)
            .then(res=>{
                if(res.data.success){
                    console.log('video dislike:'+res.data.dislike)
    ...
    return (
        <div style={{ display: 'flex', marginRight: '10px' }}>
            <div onClick={handleLike} style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                {liked ? (
                    <ThumbUpAltIcon style={{ color: 'green' }} />
                        ...
                        <ThumbUpAltOutlinedIcon style={{ color: '#888' }} />
                    )
                }
                <span style={
                    liked ? (
            ...
            <div onClick={handleDislike} style={{ marginLeft: '20px', display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                {disliked ? (
                    <ThumbDownIcon style={{ color: '#dc143c' }} />
                ) : (
                        <ThumbDownAltOutlinedIcon style={{ color: '#888' }} />
                    )
                }
                <span style={
                    disliked ? (
